...
from RBF_initializer import RBF_centers
...
if __name__ == "__main__":
    for i in range(num_ex):
        ...
        # Create a temporary generator for the training data
        ...
        # Create a new model that outputs the result of the flatten layer
        flatten_model = Model(inputs=inputs, outputs=flatten_output)
        X_train_intermediate_output = flatten_model.predict(train_generator)


        # initialization
        RBF_initialization = RBF_centers(n_components=num_enn*2)
        RBF_initialization.fit(X_train_intermediate_output,y_train.flatten())
        ...
        hidden_1 = RBFLayer(n)(flatten_output)
        ...
        RBF_weights = ([RBF_initialization.layer_1.centers,model.layers[-2].get_weights()[1]])
        model.layers[-2].set_weights(RBF_weights)

        # Create new generators for the training data
        train_generator = train_datagen.flow(
            X_train,
            y_train,
            batch_size=BATCH_SIZE,
        ...
        # Evaluation
        classification_model = load_model('test.keras', custom_objects={'ENNLayer': RBFLayer})
        ...
        if i == 0:
            graph_dict = {
            "train accuracy": history.history['categorical_accuracy'],
            "val_accuracy": history.history['val_categorical_accuracy'],
            "train_loss": history.history['loss'],
            "val_loss": history.history['val_loss'],
            }
            export_to_json(data_directory + 'Shallow_model_' + data + '_' + '_' + classification_neuron + f'_maxepochs_{epochs}_learningrate_{lr}_activation_' + activation + f'_numberofnodes_{n}_' +'graphs.json.', graph_dict)
